...
public class MembershipRepoJdbc implements MembershipRepo {
  ...
  public void save(Membership membership) {
    ...
    jdbcTemplate.update(
        query, membership.getUserId(), membership.getEntryDate(), membership.getCommunityId());
  }

  @Override
  public Optional<CommunityMemberModel> getMembership(Long userId, Long communityId) {
    String query =
        "SELECT U.login, M.entry_date, U.user_id FROM user U "
            + "INNER JOIN membership M ON U.user_id = M.user_id WHERE M.community_id = ? AND U.user_id = ?;";
    return Optional.ofNullable(
        jdbcTemplate.queryForObject(
            query, this::mapResultSetToCommunityMemberModel, communityId, userId));
  }

  private CommunityMemberModel mapResultSetToCommunityMemberModel(ResultSet rs, int rowNum)
      throws SQLException {

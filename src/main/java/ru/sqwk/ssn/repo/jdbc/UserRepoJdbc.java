...
public class UserRepoJdbc implements UserRepo {
    ...
    public List<UserModel> getUsers() {
        ...
        String query = "SELECT user_id, full_name, login, is_friends(user_id, ?) AS is_friend FROM user;";
        return jdbc.query(query, this::mapResultSetToUserModel, nowUserId);
    }
    ...
    public List<UserModel> getUsersByName(String name) {
        ...
        Long nowUserId = SecurityContextWrapper.getNowUser().getId();
        String query = "SELECT user_id, full_name, login, is_friends(user_id, ?) as is_friend FROM user WHERE" +
                "               login LIKE ?" +
                "            OR email LIKE ?" +
                "            OR full_name LIKE ?;";
        return jdbc.query(query, this::mapResultSetToUserModel, nowUserId, nameParam, nameParam, nameParam);
    }

    @Override
    public void update(Long id, UpdatedUserDTO updatedUserDTO) {
        String query = "UPDATE user SET login = ?, full_name = ?, email = ?, birthdate = ? WHERE user_id = ?;";
        jdbc.update(query, updatedUserDTO.getLogin(), updatedUserDTO.getFullName(), updatedUserDTO.getEmail(), updatedUserDTO.getBirthDate(), id);
    }
